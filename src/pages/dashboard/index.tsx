...
import type { GroceryListDoc, MealPlanDoc, TimestampLike } from "../../shared/schemas";

/** Find the most recent generation date across all plan documents. */
const latestGenerationDate = (
  meals: MealPlanDoc[],
  groceries: GroceryListDoc[]
): Date | null => {
  const allDates = [...meals, ...groceries]
    .map((docData) => docData.generatedAt)
    .filter((ts): ts is TimestampLike => !!ts)
    .map((ts) => (typeof ts.toDate === "function" ? ts.toDate() : new Date(ts as unknown as string)))
  ...
  useEffect(() => {
    const fetchLastGenerationDate = async () => {
      ...
      try {
        ...
        const latestDate = latestGenerationDate(meals, groceries);
        if (latestDate) {
        ...
        <div className="px-8 pb-8">
          ...
          {lastGeneratedDate && (
            ...
            >
              ...
              >
                {Math.floor(
                  (Date.now() - lastGeneratedDate.getTime()) / (1000 * 60 * 60 * 24)
                )}{" "}
                days ago
              </div>
            </motion.div>
