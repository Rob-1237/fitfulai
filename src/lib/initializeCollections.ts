...
import type { Macros, UserPreferences, UserProfile } from '../shared/schemas';

/** Data collected by the onboarding wizard, handed to collection setup. */
 ...
 */
export const initializeUserCollections = async (
  userId: string,
 ...
 * Includes body metrics, nutrition goals, and calculated targets
 */
const createUserProfile = async (
  ...
  const profileData = {
    ...
    // Nutrition goal and activity (fitnessGoal is the legacy field name)
    fitnessGoal: onboardingData.fitnessGoal ?? null, // e.g., 'weight_loss', 'muscle_gain'
    activityLevel: onboardingData.activityLevel ?? null, // e.g., 'sedentary', 'light', 'moderate'
    ...
    updatedAt: serverTimestamp(),
  } satisfies Record<keyof UserProfile | string, unknown>;

  const profileRef = doc(db, 'users', userId);
  await setDoc(profileRef, profileData);
  console.log('✅ Profile document created with complete onboarding data');
};

/**
 * Create meals collection structure with a placeholder document
 */
const createMealsCollection = async (userId: string): Promise<void> => {
  ...
  const placeholderMealPlan = {
    ...
    // Weekly meal structure — only Monday populated for the placeholder;
    // the AI generates the full week
    days: {
      monday: {
        breakfast: {
          ...
          ingredients: [{ name: 'Sample ingredient', amount: 1, unit: 'cup' }],
        ...
        lunch: {
          ...
          ingredients: [{ name: 'Sample ingredient', amount: 1, unit: 'serving' }],
          instructions: ['Sample instruction'],
          prepTime: 15,
          cookTime: 10,
          servings: 1,
        ...
        dinner: {
          ...
          ingredients: [{ name: 'Sample ingredient', amount: 1, unit: 'portion' }],
          instructions: ['Sample instruction'],
          prepTime: 20,
          cookTime: 25,
          servings: 1,
        ...
        snacks: [
          {
            ...
            ingredients: [{ name: 'Sample snack item', amount: 1, unit: 'serving' }],
...
/**
 * Create groceries collection with placeholder document
 */
const createGroceriesCollection = async (userId: string): Promise<void> => {
 ...
 */
const initializeAiCache = async (userId: string): Promise<void> => {
 ...
 */
export const userCollectionsExist = async (userId: string): Promise<boolean> => {
  try {
    const profileRef = doc(db, 'users', userId);
    const profileDoc = await getDoc(profileRef);
 ...
 */
export const cleanupPlaceholders = async (_userId: string): Promise<void> => {
