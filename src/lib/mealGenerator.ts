...
import type { AiUsage, GenerationResult, MealPlan, MealPlanDoc, UserProfile } from '../shared/schemas';
...
const getWeekDate = (weekStartDate: Date, dayOffset: number): string => {
...
export const generateMealPlan = async (
  ...
  planType = 'weekly',
  forceRefresh = false
): Promise<GenerationResult<MealPlanDoc>> => {
  ...
  try {
    ...
    if (cachedResponse) {
      ...
      const processedMealPlan = await saveMealPlanToFirestore(
        userId,
        cachedResponse.response as MealPlan,
        {
          promptTokens: 0,
          completionTokens: 0,
          totalTokens: 0,
...
const buildMealPlanPrompt = (userProfile: UserProfile, planType: string): string => {
  const { age, gender, fitnessGoal, activityLevel, dietaryPreferences = [], allergies = [] } = userProfile;
...
4. Consider their nutrition goal (${fitnessGoal})
...
const saveMealPlanToFirestore = async (
  userId: string,
  ...
  try {
    ...
    return mealPlanDoc as unknown as MealPlanDoc;
  } catch (error) {
...
const removeMealPlanPlaceholder = async (userId: string): Promise<void> => {
  try {
...
// needs. Strict zod validation (MealPlanSchema) moves server-side on Day 4
// when structured outputs guarantee the full schema.
function validateMealPlanData(mealPlanData: unknown): asserts mealPlanData is MealPlan {
  if (typeof mealPlanData !== 'object' || mealPlanData === null) {
    throw new Error('Meal plan response is not an object');
  }

  const data = mealPlanData as Record<string, unknown>;
