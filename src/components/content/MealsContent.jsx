...
export default function MealsContent({ isDark }) {
  ...
  const todaysMeals = currentMealPlan?.days?.[today]?.meals || {};

  // Get all days for the displayed week
  const weekDays = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'];
  const displayedWeekMeals = weekDays.map(day => ({
    day,
  ...
  const handleRecipeClick = (recipe, mealType) => {
  ...
  const toggleMealSkipped = async (mealType, day = today) => {
    if (!currentMealPlan?.id) return;
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {(() => {
        ...
        let message = '';
        let urgency = 'info'; // 'info', 'warning', 'urgent'
