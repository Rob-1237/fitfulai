...
export default function MealsContent({ isDark }) {
  ...
  console.log('MealsContent Debug:', {
    mealPlansLength: mealPlans.length,
    mealPlans: mealPlans.map(p => ({ id: p.id, type: p.type, name: p.name })),
    shouldShowButton: mealPlans.every(p => p.generatedBy === 'system' || p.aiPrompt === 'Placeholder meal plan'),
    isLoading,
    userProfile: !!userProfile
  });

  // More detailed meal plan info
  if (mealPlans.length > 0) {
    console.log('First meal plan details:', mealPlans[0]);
    console.log('First meal plan type specifically:', mealPlans[0].type);
    console.log('Type check result:', mealPlans[0].type === 'placeholder');
  }
  ...
  const isPlaceholder = currentMealPlan?.generatedBy === 'system' || currentMealPlan?.aiPrompt === 'Placeholder meal plan';
            ...
            {mealPlans.map((plan, index) => (
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  ...
                  {(plan.generatedBy === 'system' || plan.aiPrompt === 'Placeholder meal plan') && (
                    <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-1 rounded-full">
                      Sample
                    </span>
                  )}
