...
export default function OnboardingWizard({ open, onClose }: OnboardingWizardProps) {
  ...
  const steps: WizardStep[] = [
    ...
    { id: 3, title: 'Nutrition Goals', component: FitnessGoalsStep },
  ...
  useEffect(() => {
    ...
    if (draft) {
      try {
        const parsed = JSON.parse(draft) as OnboardingData;
  ...
  const validateCurrentStep = (): boolean => {
    ...
    switch (currentStep) {
      ...
      case 2: {
        // Physical Stats - Apply validation thresholds
        const { heightFeet, heightInches, heightCm, weightLbs, weightKg } = onboardingData;
        if (onboardingData.unitsPreference === 'imperial') {
          const heightFeetValid = heightFeet !== null && heightFeet >= 3 && heightFeet <= 8;
          // Allow 0 as valid inches value (e.g., 6'0")
          const heightInchesValid = heightInches !== null && heightInches >= 0 && heightInches <= 11;
          const totalInches = (heightFeet ?? 0) * 12 + (heightInches || 0);
          const totalHeightValid = totalInches >= 36 && totalInches <= 96;
          const weightValid = weightLbs !== null && weightLbs >= 50 && weightLbs <= 1000;
          return heightFeetValid && heightInchesValid && totalHeightValid && weightValid;
        } else {
          const heightValid = heightCm !== null && heightCm >= 91 && heightCm <= 244;
          const weightValid = weightKg !== null && weightKg >= 22 && weightKg <= 453;
          return heightValid && weightValid;
        }
      }
      case 3: // Nutrition Goals
