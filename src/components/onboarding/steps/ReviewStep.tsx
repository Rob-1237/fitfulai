...
export default function ReviewStep({ data, updateData, isLoading, setIsLoading, onComplete }: StepProps) {
  const { user } = useAuth();
  ...
  const calculateMetrics = () => {
    ...
    const weightKg = data.weightKg || (data.weightLbs ?? 0) * 0.453592;
    const heightCm = data.heightCm || ((data.heightFeet ?? 0) * 12 + (data.heightInches ?? 0)) * 2.54;
  ...
  const handleGeneratePlans = async () => {
    // Step 5 is only reachable once earlier steps validate, so these should
    // always be present — guard anyway rather than generate with bad data.
    if (!user || !metrics) {
      console.warn('⚠️ Cannot generate plans: missing user or calculated metrics');
      return;
    }
    ...
    try {
      ...
      const completeOnboardingData = {
        ...
        weightKgs: data.weightKg ?? undefined,
        heightInches: data.unitsPreference === 'imperial'
          ? ((data.heightFeet ?? 0) * 12 + (data.heightInches ?? 0))
          : Math.round((data.heightCm ?? 0) / 2.54),
        heightCentimeters: data.heightCm || Math.round(((data.heightFeet ?? 0) * 12 + (data.heightInches ?? 0)) * 2.54),
      ...
      if (result.success) {
        ...
        if (onComplete) {
          onComplete(completeUserProfile as UserProfile);
        }
      } else {
      ...
      alert(`Sorry, there was an error completing your profile: ${message}. Please try again.`);
      setIsLoading(false);
    }
  };
