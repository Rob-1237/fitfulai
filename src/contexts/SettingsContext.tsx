...
interface SettingsContextValue {
  ...
  isLoading: boolean;
  isSynced: boolean;
  /** Update a setting by key; supports dotted paths like 'notifications.email' */
...
export const SettingsProvider = ({ children, userId }: SettingsProviderProps) => {
  ...
  const saveToFirestore = useCallback(async (uid: string, newSettings: Settings) => {
    if (!uid) return;
  ...
  const updateSetting = useCallback(
    async (key: string, value: unknown) => {
      ...
      if (key.includes('.')) {
        ...
        (newSettings as unknown as Record<string, unknown>)[key] = value;
      }
  ...
  const value = useMemo<SettingsContextValue>(
    ...
    [settings, isLoading, isSynced, updateSetting, toggleTheme, resetSettings]
  );
