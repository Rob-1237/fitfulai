...
import type { MealPlanDoc } from '../shared/schemas';
 ...
 * - Generation: Only when few days remain in current plan
 */

export type WeekStartDay = 'sunday' | 'monday';

// Helper: Get Sunday of the week containing the given date
const getWeekStart = (date: Date | string, weekStartDay: WeekStartDay = 'sunday'): Date => {
...
export const useWeekContext = (weekStartDay: WeekStartDay = 'sunday') => {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- only run when the selection changes; searchParams identity churns on every render
  }, [selectedWeek]);
  ...
  const isFutureWeek = useMemo(() => selectedWeek > currentWeek, [selectedWeek, currentWeek]);
  ...
  const goToWeek = (date: Date | string) => {
  ...
  const getDaysRemainingInPlan = (mealPlan: MealPlanDoc | null | undefined): number => {
  ...
  // Helper: Can user generate a plan? (<=3 days remain)
  const canGeneratePlan = (currentMealPlan: MealPlanDoc | null | undefined): boolean => {
  ...
  return {
    ...
    isDateInPast,
    isToday,
    getWeekStart: (date: Date | string) => getWeekStart(date, weekStartDay),
